...
XE는 아래 목록과 같이 이미 다양한 UI오브젝트를 제공하고 있습니다. XE에서 기본으로 제공하는 UI오브젝트 이외에 여러 플러그인이 등록한 UI오브젝트도 있을수 있습니다.
...
폼은 웹페이지에서 사용자에게 입력을 받기 위해 매우 빈번히 사용됩니다. 특히 테마나 위젯, 스킨과 같은 컴포넌트는 사이트 관리자에게 폼으로 구성되어 있는 설정 페이지를 제공합니다. UI오브젝트 중 아이디가 `form*` 형식인 UI오브젝트를 폼 UI오브젝트라고 합니다. 폼 UI오브젝트를 사용하면 손쉽게 폼의 필드를 출력할 수 있습니다.
