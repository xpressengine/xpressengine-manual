...
XE는 아래 목록과 같이 이미 다양한 UI오브젝트를 제공하고 있습니다. XE에서 기본으로 제공하는 UI오브젝트 이외에 여러 플러그인에서도 UI오브젝트를 추가할 수 있습니다.


## UI오브젝트의 출력

`uio()` 함수를 사용하여 UI오브젝트를 출력하십시오. 선택된 UI오브젝트는 알아서 html 마크업을 생성하여 출력하고, 스타일시트와 스크립트 파일도 자동으로 로드합니다.
...
두번째 파라미터 `$args`는 UI오브젝트를 출력하기 위해 전달할 정보를 담은 배열입니다. 각 UI오브젝트마다 전달하는 정보의 형식이 상이합니다.
