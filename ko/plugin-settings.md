...
- `link`는 메뉴를 클릭했을 때 이동하는 주소입니다. 링크가 없다면 적지 않아도 됩니다. 또, 여기에 직접 `link`를 지정하지 않는 것을 추천합니다. 대신 아래 '라우트에 메뉴 연결하기' 절에서 안내하는 방식을 사용하십시오.
...
사이트 관리 메뉴를 등록할 때, `link` 정보를 직접 지정하는 것을 추천하지 않습니다. 아래의 방법으로 라우트에 메뉴를 연결할
