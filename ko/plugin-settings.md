...
- `link`는 메뉴를 클릭했을 때 이동하는 주소입니다. 링크가 없다면 적지 않아도 됩니다. 또, 여기에 직접 `link`를 지정하지 않는 것을 추천합니다. 대신 아래 '라우트에 메뉴 연결하기' 절에서 안내하는 방식을 사용하십시오. 그 이유는, '회원정보 수정'과 같이 라우트 파라미터를 가지는 관리페이지의 경우, 메뉴에 `link`를 지정할 수 없기 때문입니다.
