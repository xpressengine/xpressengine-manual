...
위와 같이 권한을 등록해 놓으면, '사이트 관리 > 설정 > 관리페이지 권한 설정' 페이지에 등록한 권한이 표시됩니다. 사이트 관리자는 이 페이지에서 권한을 부여하고 싶은 사용자를 설정합니다.


등록한 권한을 라우트와 연결하는 과정도 메뉴에서 사용한 방식과 유사합니다. `settings_permission` 필드에 권한의 아이디를 지정하십시오.
