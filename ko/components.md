...
테마는 사이트의 디자인을 결정짓는 가장 결정적인 역할을 담당합니다. 웹 페이지는 요청된 URL에 따라 다른 콘텐츠를 출력합니다. 이때, URL에 따라 달라지는 콘텐츠가 아닌 공통적으로 출력되는 영역이 있습니다. 테마는 이런 공통적인 영역을 출력하는 역할을 합니다. 따라서 `header`, `footer`, `content`, `sidebar`와 같은 영역으로 구성되는 레이아웃을 결정하기도 
