...
웹브라우저가 XE사이트로 요청을 보내면, `index.php`가 실행되고, `index.php`는 현재 요청에 대한 정보를 담고 있는 `Request` 인스턴스를 생성합니다. 이 `Request`인스턴스는 XE가 실행되는 동안 매우 많은 곳에서 로드되어 현재 요청에 대한 정보를 참조할 수 있도록 합니다.
