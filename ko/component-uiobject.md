...
위 별칭 목록 중에 `form*`의 형식으로 등록된 UI오브젝트를 폼 UI오브젝트라 합니다. 폼 UI오브젝트는 말 그대로 폼을 구성할 때 사용할 수 있는 UI오브젝트들입니다. 폼 UI오브젝트는 테마나 스킨, 위젯와 같은 컴포넌트가 사이트 관리자에게 설정 폼을 출력할 때 사용됩니다. 폼 UI오브젝트의 사용법은 [폼 출력](service-form.md) 문서에서 자세히 설명합니다.
